...
e.substring(j-p-1);return[q].concat(P(e,j))}var c=this;c.addClass("cmd");c.append('<span class="prompt"></span><span></span><span class="cursor">&nbsp;</span><span></span>');var i=g("<textarea/>").addClass("clipboard").appendTo(c);a.width&&c.width(a.width);var j,p,u=a.mask||false,k="",m=0,C,E=a.enabled,K,z,L=function(){var e=c.find(".cursor");return function(){e.toggleClass("inverted")}}(),N=c.find(".cursor"),f=function(e){function q(b,h){if(h==b.length){w.html(x(b));D.html("&nbsp;");o.html("")}else if(h===
...
y[B].length;return t}(h.slice(0,r));n=m-n;if(n==j){n=0;s=h[++r]}x(s.slice(0,n));q(s,n);G(h.slice(0,r));l(h.slice(r+1))}}}}else if(b===""){w.html("");D.html("&nbsp;");o.html("")}else q(b,m)}}(c),F=function(){var e=c.find(".prompt");return function(){if(typeof C=="string"){p=C.length;e.html(x(C)+"&nbsp;")}else C(function(q){p=q.length;e.html(x(q)+"&nbsp;")})}}();g.extend(c,{name:function(e){if(e!==v){K=e;z=new $(e)}else return K},history:function(){return z},set:function(e,q){if(e!==v){k=e;if(!q)m=
k.length;f()}},insert:function(e,q){if(m==k.length)k+=e;else k=m===0?e+k:k.slice(0,m)+e+k.slice(m);q||(m+=e.length);f()},get:function(){return k},commands:function(e){if(e)a.commands=e;else return e},destroy:function(){g(document.documentElement).unbind(".commandline");c.find(".prompt").remove()},prompt:function(e){if(e===v)return C;else{if(typeof e=="string"||typeof e=="function")C=e;else throw"prompt must be a function or string";F()}},position:function(e){if(typeof e=="number"){m=e<0?0:e>k.length?
k.length:e;f()}else return m},resize:function(e){if(e)j=e;else{e=c.width();var q=N.innerWidth();j=Math.floor(e/q)}f()},enable:function(){if(!E){c.everyTime(500,"blink",L);E=true}},isenabled:function(){return E},disable:function(){if(E){c.stopTime("blink",L);c.find(".cursor").removeClass("inverted");E=false}},mask:function(e){if(typeof e=="boolean"){u=e;f()}else return u}});c.name(a.name||"");C=a.prompt||">";F();if(a.enabled===v||a.enabled===true)c.enable();g(document.documentElement).keypress(function(e){var q;
if(e.ctrlKey&&e.which==99)return true;if(a.keypress)q=a.keypress(e);if(q===v||q){if(E)if([38,32,13,0,8].has(e.which)&&e.keyCode!=123&&!(e.which==38&&e.shiftKey))return false;else if(!e.ctrlKey&&!(e.altKey&&e.which==100)){c.insert(String.fromCharCode(e.which));return false}}else return q;if(e.which==100&&e.ctrlKey)return false}).keydown(function(e){if(a.keydown&&a.keydown(e)===false)return false;if(E){var q;if(e.keyCode==13){z&&k&&z.append(k);z.last();e=k;c.set("");typeof C=="function"&&F();a.commands&&
a.commands(e)}else if(e.which==32)c.insert(" ");else if(e.which==8){if(k!==""&&m>0){k=k.slice(0,m-1)+k.slice(m,k.length);--m;f()}}else if(e.which==9&&!(e.ctrlKey||e.altKey))c.insert("\t");else if(e.which==46||e.which==68&&e.ctrlKey){if(k!==""&&m<k.length){k=k.slice(0,m)+k.slice(m+1,k.length);f()}return true}else if(z&&e.which==38||e.which==80&&e.ctrlKey)c.set(z.previous());else if(z&&e.which==40||e.which==78&&e.ctrlKey)c.set(z.next());else if(e.which==27)c.set("");else if(e.which==37||e.which==66&&
e.ctrlKey)if(e.ctrlKey&&e.which!=66){q=m-1;e=0;for(k[q]==" "&&--q;q>0;--q)if(k[q]==" "&&k[q+1]!=" "){e=q+1;break}c.position(e)}else{if(m>0){--m;f()}}else if(e.which==39||e.which==70&&e.ctrlKey)if(e.ctrlKey&&e.which!=70){k[m]==" "&&++m;e=k.slice(m).match(/[^ ] {2,}| +[^ ]?/);if(!e||e[0].match(/^ +$/))m=k.length;else if(e[0][0]!=" ")m+=e.index+1;else{m+=e.index+e[0].length-1;e[0][e[0].length-1]!=" "&&--m}f()}else{if(m<k.length){++m;f()}}else if(e.which==123)return true;else if(e.which==36)c.position(0);
else if(e.which==35)c.position(k.length);else if(e.ctrlKey)if(e.shiftKey){if(e.which==84)return true}else{if(!e.altKey)if(e.which==65)c.position(0);else if(e.which==69)c.position(k.length);else if(e.which==88||e.which==67||e.which==87||e.which==84)return true;else if(e.which==86){i.focus();c.oneTime(1,function(){c.insert(i.val());i.blur();i.val("")});return true}else if(e.which==75)if(m===0)c.set("");else m!=k.length&&c.set(k.slice(0,m));else if(e.which==17)return true}else if(e.altKey)e.which==68&&
c.set(k.slice(0,m)+k.slice(m).replace(/[^ ]+ |[^ ]+$/,""),true);else return true;return false}else if(e.altKey&&e.which==68||e.ctrlKey&&[65,66,68,69,80,78,70].has(e.which)||[35,36,37,38,39,40].has(e.which))return false});return c};var M=[];g.jrpc=function(a,d,c,i,j,p){d=g.json_stringify({jsonrpc:"2.0",method:c,params:i,id:d});return g.ajax({url:a,data:d,success:j,error:p,contentType:"application/json",dataType:"json",beforeSend:function(u){M.push(u)},async:true,cache:false,type:"POST"})};J=/ {13}$/;
...
9&&b.ctrlKey)f.focus(false);else if(b.keyCode==34)f.scroll(f.height());else b.keyCode==33?f.scroll(-f.height()):f.attr({scrollTop:f.attr("scrollHeight")})}var f=this,F=[],e,q=H.length(),A,l={name:null,prompt:">",history:true,exit:true,clear:true,enabled:true,login:null,onInit:null,onExit:null,keypress:null,keydown:null};if(d){d.width&&f.width(d.width);d.height&&f.height(d.height);g.extend(l,d)}var G=!l.enabled;if(f.length===0)throw'Sorry, but terminal said that "'+f.selector+'" is not valid selector';
if(f.data("terminal")){f.ajaxSend(function(b,h){M.push(h)});return f.data("terminal")}e=g("<div>").addClass("terminal-output").appendTo(f);f.addClass("terminal").append("<div/>");g.extend(f,{clear:function(){e.html("");o.set("");F=[];f.attr({scrollTop:0});return f},paused:function(){return G},pause:function(){if(o){f.disable();o.hide()}return f},resume:function(){if(o){f.enable();o.show();p(f)}return f},cols:function(){return A},rows:function(){return F.length},history:function(){return o.history().data()},
...
signature:function(){var b=f.cols();b=b<15?null:b<35?0:b<55?1:b<64?2:b<75?3:4;return b!==null?ba[b].join("\n")+"\n":""},get_command:function(){return o.get()},insert:function(b){o.insert(b);return f},set_prompt:function(b){j("prompt",b)&&o.prompt(b);return f},set_command:function(b){o.set(b);return f},set_mask:function(b){o.mask(b);return f},get_output:function(){return g.map(F,function(b,h){return typeof h=="function"?h():h}).get().join("\n")},resize:function(b,h){if(b&&h){f.width(b);f.height(h)}A=
c();o.resize(A);var n=e.detach();e.html("");g.each(F,function(s,r){u(typeof r=="function"?r():r)});f.prepend(n);p(f);return f},echo:function(b){F.push(b);return u(typeof b=="function"?b():b)},error:function(b){f.echo(b).addClass("error")},scroll:function(b){b>f.attr("scrollTop")&&b>0&&f.attr("scrollTop",0);var h=f.attr("scrollTop");f.attr("scrollTop",h+b);return f},logout:l.login?function(){for(;w.size()>1;)w.pop();K();return f}:function(){throw"You don't have login function";},token:l.login?function(){var b=
