...
cache:false,type:"POST"})};J=/ {13}$/;var ba=[["jQuery Terminal","(c) 2011 jcubic"],["JQuery Terminal Emulator v. 0.3.3","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>".replace(/ *<.*>/,"")],["JQuery Terminal Emulator version version 0.3.3","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"],["      _______                 ________                        __","     / / _  /_ ____________ _/__  ___/______________  _____  / /"," __ / / // / // / _  / _/ // / / / _  / _/     / /  \\/ / _ \\/ /",
"/  / / // / // / ___/ // // / / / ___/ // / / / / /\\  / // / /__","\\___/____ \\\\__/____/_/ \\__ / /_/____/_//_/ /_/ /_/  \\/\\__\\_\\___/","         \\/          /____/                                   ".replace(J,"")+"version 0.3.3","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"],["      __ _____                     ________                              __","     / // _  /__ __ _____ ___ __ _/__  ___/__ ___ ______ __ __  __ ___  / /"," __ / // // // // // _  // _// // / / // _  // _//     // //  \\/ // _ \\/ /",
...
118&&b.ctrlKey){f.oneTime(1,function(){f.attr({scrollTop:f.attr("scrollHeight")})});return true}else if(b.keyCode==9&&b.ctrlKey)f.focus(false);else if(b.keyCode==34)f.scroll(f.height());else b.keyCode==33?f.scroll(-f.height()):f.attr({scrollTop:f.attr("scrollHeight")})}var f=this,E=[],e,q=H.length(),A,l={name:null,prompt:">",history:true,exit:true,clear:true,enabled:true,login:null,onInit:null,onExit:null,keypress:null,keydown:null};if(d){d.width&&f.width(d.width);d.height&&f.height(d.height);g.extend(l,
d)}var G=!l.enabled;if(f.length===0)throw'Sorry, but terminal said that "'+f.selector+'" is not valid selector';if(f.data("terminal")){f.ajaxSend(function(b,h){M.push(h)});return f.data("terminal")}e=g("<div>").addClass("terminal-output").appendTo(f);f.addClass("terminal").append("<div/>");g.extend(f,{clear:function(){e.html("");o.set("");E=[];f.attr({scrollTop:0});return f},paused:function(){return G},pause:function(){if(o){f.disable();o.hide()}return f},resume:function(){if(o){f.enable();o.show();
p(f)}return f},cols:function(){return A},rows:function(){return E.length},history:function(){return o.history().data()},next:function(){if(H.length()==1)return f;else{H.front().disable();var b=H.rotate().enable(),h=b.offset().top-50;g("html,body").animate({scrollTop:h},500);return b}},focus:function(b){f.oneTime(1,function(){if(H.length()==1)b===false?f.disable():f.enable();else if(b===false)f.next();else{H.front().disable();H.set(f);f.enable()}});return f},enable:function(){A===v&&f.resize();if(G)if(o){o.enable();
G=false}return f},disable:function(){if(o){G=true;o.disable()}return f},enabled:function(){return G},signature:function(){var b=f.cols();b=b<15?null:b<35?0:b<55?1:b<64?2:b<75?3:4;return b!==null?ba[b].join("\n")+"\n":""},get_command:function(){return o.get()},insert:function(b){o.insert(b);return f},set_prompt:function(b){j("prompt",b)&&o.prompt(b);return f},set_command:function(b){o.set(b);return f},set_mask:function(b){o.mask(b);return f},get_output:function(){return g.map(E,function(b,h){return typeof h==
"function"?h():h}).get().join("\n")},resize:function(b,h){if(b&&h){f.width(b);f.height(h)}A=c();o.resize(A);var n=e.detach();e.html("");g.each(E,function(s,r){u(typeof r=="function"?r():r)});f.prepend(n);p(f);return f},echo:function(b){E.push(b);return u(typeof b=="function"?b():b)},error:function(b){f.echo(b).addClass("error")},scroll:function(b){b>f.attr("scrollTop")&&b>0&&f.attr("scrollTop",0);var h=f.attr("scrollTop");f.attr("scrollTop",h+b);return f},logout:l.login?function(){for(;w.size()>1;)w.pop();
K();return f}:function(){throw"You don't have login function";},token:l.login?function(){var b=l.name;return g.Storage.get("token"+(b?"_"+b:""))}:null,login_name:l.login?function(){var b=l.name;return g.Storage.get("login_"+(b?"_"+b:""))}:null,name:function(){return l.name},push:function(b,h){if(!h.prompt||j("prompt",h.prompt)){if(typeof b=="string")b=k(h.eval,f);w.push(g.extend({eval:b},h));z()}return f},pop:function(b){b!==v&&m(b);if(w.top().name===l.name){if(l.login){K();if(typeof l.onExit=="function")l.onExit(f)}}else{b=
w.pop();z();if(typeof b.onExit=="function")b.onExit(f)}return f}});var D;switch(typeof a){case "string":D=a;a=k(a,f);break;case "object":a=function(b){return function(h){if(h!=""){h=h.split(/ */);var n=h[0];h=h.slice(1);var s=b[n];typeof s=="function"?s.apply(f,h):f.echo("Command '"+n+"' Not Found")}}}(a)}if(D&&typeof l.login=="string"||D)l.login=function(b){var h=1;return function(n,s,r){f.pause();g.jrpc(D,h++,b,[n,s],function(y){f.resume();!y.error&&y.result?r(y.result):r(null)},function(y,t){f.resume();
f.error("&#91;AJAX&#92; Response: "+t+"\n"+y.responseText)})}}(typeof l.login=="boolean"?"login":l.login);if(j("prompt",l.prompt)){var w=new Z({name:l.name,eval:a,prompt:l.prompt,greetings:l.greetings}),o=f.find(".terminal-output").next().cmd({prompt:l.prompt,history:l.history,width:"100%",keydown:l.keydown?function(b){return l.keydown(b,f)}:null,keypress:N,commands:C});f.livequery(function(){f.resize()});H.append(f);l.enabled===true?f.focus():f.disable();g(window).resize(f.resize);f.click(function(){f.focus()});
f.token&&!f.token()&&f.login_name&&!f.login_name()?F():L();typeof g.fn.init.prototype.mousewheel==="function"&&f.mousewheel(function(b,h){h>0?f.scroll(-40):f.scroll(40);return false},true)}f.data("terminal",f);return f}})(jQuery);
