...
if(e.ctrlKey&&e.which==99)return true;if(a.keypress)q=a.keypress(e);if(q===v||q){if(F)if([38,32,13,40,0,8].has(e.which)&&e.keyCode!=123&&!(e.which==40&&e.shiftKey||e.which==38&&e.shiftKey))return false;else if(!e.ctrlKey&&!(e.altKey&&e.which==100)){c.insert(String.fromCharCode(e.which));if(e.which==45||e.shiftKey&&e.which==43)return false}}else return q;if(e.which==100&&e.ctrlKey)return false}).keydown(function(e){if(a.keydown&&a.keydown(e)===false)return false;if(F){var q;if(e.keyCode==13){z&&k&&
z.append(k);z.last();e=k;c.set("");typeof C=="function"&&E();a.commands&&a.commands(e)}else if(e.which==32)c.insert(" ");else if(e.which==8){if(k!==""&&m>0){k=k.slice(0,m-1)+k.slice(m,k.length);--m;f()}}else if(e.which==9&&!(e.ctrlKey||e.altKey))c.insert("\t");else if(e.which==46||e.which==68&&e.ctrlKey){if(k!==""&&m<k.length){k=k.slice(0,m)+k.slice(m+1,k.length);f()}return true}else if(z&&e.which==38||e.which==80&&e.ctrlKey)c.set(z.previous());else if(z&&e.which==40||e.which==78&&e.ctrlKey)c.set(z.next());
else if(e.which==27)c.set("");else if(e.which==37||e.which==66&&e.ctrlKey)if(e.ctrlKey&&e.which!=66){q=m-1;e=0;for(k[q]==" "&&--q;q>0;--q)if(k[q]==" "&&k[q+1]!=" "){e=q+1;break}c.position(e)}else{if(m>0){--m;f()}}else if(e.which==39||e.which==70&&e.ctrlKey)if(e.ctrlKey&&e.which!=70){k[m]==" "&&++m;e=k.slice(m).match(/[^ ] {2,}| +[^ ]?/);if(!e||e[0].match(/^ +$/))m=k.length;else if(e[0][0]!=" ")m+=e.index+1;else{m+=e.index+e[0].length-1;e[0][e[0].length-1]!=" "&&--m}f()}else{if(m<k.length){++m;f()}}else if(e.which==
123)return true;else if(e.which==36)c.position(0);else if(e.which==35)c.position(k.length);else if(e.ctrlKey)if(e.shiftKey){if(e.which==84)return true}else{if(!e.altKey)if(e.which==65)c.position(0);else if(e.which==69)c.position(k.length);else if(e.which==88||e.which==67||e.which==87||e.which==84)return true;else if(e.which==86){i.focus();c.oneTime(1,function(){c.insert(i.val());i.blur();i.val("")});return true}else if(e.which==75)if(m===0)c.set("");else m!=k.length&&c.set(k.slice(0,m));else if(e.which==
17)return true}else if(e.altKey)e.which==68&&c.set(k.slice(0,m)+k.slice(m).replace(/[^ ]+ |[^ ]+$/,""),true);else return true;return false}else if(e.altKey&&e.which==68||e.ctrlKey&&[65,66,68,69,80,78,70].has(e.which)||[35,36,37,38,39,40].has(e.which))return false});return c};var M=[];g.jrpc=function(a,d,c,i,j,p){d=g.json_stringify({jsonrpc:"2.0",method:c,params:i,id:d});return g.ajax({url:a,data:d,success:j,error:p,contentType:"application/json",dataType:"json",beforeSend:function(u){M.push(u)},async:true,
cache:false,type:"POST"})};J=/ {13}$/;var ba=[["jQuery Terminal","(c) 2011 jcubic"],["JQuery Terminal Emulator v. 0.3.2","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>".replace(/ *<.*>/,"")],["JQuery Terminal Emulator version version 0.3.2","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"],["      _______                 ________                        __","     / / _  /_ ____________ _/__  ___/______________  _____  / /"," __ / / // / // / _  / _/ // / / / _  / _/     / /  \\/ / _ \\/ /",
"/  / / // / // / ___/ // // / / / ___/ // / / / / /\\  / // / /__","\\___/____ \\\\__/____/_/ \\__ / /_/____/_//_/ /_/ /_/  \\/\\__\\_\\___/","         \\/          /____/                                   ".replace(J,"")+"version 0.3.2","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"],["      __ _____                     ________                              __","     / // _  /__ __ _____ ___ __ _/__  ___/__ ___ ______ __ __  __ ___  / /"," __ / // // // // // _  // _// // / / // _  // _//     // //  \\/ // _ \\/ /",
"/  / // // // // // ___// / / // / / // ___// / / / / // // /\\  // // / /__","\\___//____ \\\\___//____//_/ _\\_  / /_//____//_/ /_/ /_//_//_/ /_/ \\__\\_\\___/","          \\/              /____/                                          ".replace(J,"")+"version 0.3.2","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"]],H=new function(a){var d=a?[a]:[],c=0;g.extend(this,{rotate:function(){if(d.length==1)return d[0];else{if(c==d.length-1)c=0;else++c;return d[c]}},length:function(){return d.length},
set:function(i){for(var j=d.length;j--;)if(d[j]===i){c=j;return}this.append(i)},front:function(){return d[c]},append:function(i){d.push(i)}})};M=[];g.fn.terminal=function(a,d){function c(){var b=g("<span>x</span>").appendTo(f),h=Math.floor(f.width()/b.width());b.remove();return h}function i(b,h){if(typeof b=="string")f.error("&#91;"+h+"&#93;: "+b);else{f.error("&#91;"+h+"&#93;: "+b.fileName+": "+b.message);f.pause();g.get(b.fileName,function(n){f.resume();var s=b.lineNumber-1;f.error("&#91;"+b.lineNumber+
"&#93;: "+n.split("\n")[s])})}}function j(b,h){try{if(typeof h=="function")h(function(){});else if(typeof h!="string")throw b+" must be string or function";}catch(n){i(n,b.toUpperCase());return false}return true}function p(b){b.scrollTop(f.attr("scrollHeight"))}function u(b){b=typeof b=="string"?b:String(b);var h;if(b.length>A){b=b.split("\n");h=g("<div></div>");for(var n=b.length,s=0;s<n;++s)if(b[s]===""||b[s]=="\r")h.append("<div>&nbsp;</div>");else if(b[s].length>A){var r=P(b[s],A);g.each(r,function(y,
t){g("<div/>").html(x(t)).appendTo(h)})}else g("<div/>").html(x(b[s])).appendTo(h)}else h=g("<div/>").html(x(b));e.append(h);h.width("100%");p(f);return h}function k(b,h){var n=1,s=function(r,y){h.pause();g.jrpc(b,n++,r,y,function(t){if(t.error)h.error("&#91;RPC&#93; "+t.error.message);else if(typeof t.result=="string")h.echo(t.result);else if(t.result instanceof Array)h.echo(t.result.join(" "));else if(typeof t.result=="object"){var B="",I;for(I in t.result)if(t.result.hasOwnProperty(I))B+=I+": "+
t.result[I]+"\n";h.echo(B)}h.resume()},function(t,B){h.error("&#91;AJAX&#93; "+B+" - Server reponse is: \n"+t.responseText);h.resume()})};return function(r,y){if(r!==""){var t,B;if(r.match(/[^ ]* /)){r=r.split(/ */);t=r[0];B=r.slice(1)}else{t=r;B=[]}if(!l.login||t=="help")s(t,B);else{var I=y.token();I?s(t,[I].concat(B)):y.error("&#91;AUTH&#93; Access denied (no token)")}}}}function m(b){var h=o.prompt();if(o.mask())b=b.replace(/./g,"*");typeof h=="function"?h(function(n){f.echo(n+" "+b)}):f.echo(h+
" "+b)}function C(b){try{var h=w.top();if(b=="exit"&&l.exit)if(w.size()==1)l.login?K():f.echo("You can exit from main interpeter");else f.pop("exit");else{m(b);b=="clear"&&l.clear?f.clear():h.eval(b,f)}}catch(n){i(n,"USER");throw n;}}function F(){var b=null;o.prompt("login:");l.history&&o.history().disable();o.commands(function(h){try{m(h);if(b){o.mask(false);f.pause();l.login(b,h,function(s){if(s){var r=l.name;r=r?"_"+r:"";g.Storage.set("token"+r,s);g.Storage.set("login"+r,b);o.commands(C);L()}else{f.error("Wrong password try again");
o.prompt("login:");b=null}f.resume();l.history&&o.history().enable()})}else{b=h;o.prompt("password:");o.mask(true)}}catch(n){i(n,"LOGIN",f);throw n;}})}function K(){var b=l.name;b=b?"_"+b:"";g.Storage.remove("token"+b,null);g.Storage.remove("login"+b,null);l.history&&o.history().disable();F()}function z(){var b=w.top(),h="";if(b.name!==v&&b.name!=="")h+=b.name+"_";h+=q;o.name(h);o.prompt(b.prompt);l.history&&o.history().enable();o.set("");if(typeof b.onStart=="function")b.onStart(f)}function L(){z();
if(d.greetings===v)f.echo(f.signature);else d.greetings&&f.echo(d.greetings);if(typeof l.onInit=="function")l.onInit(f)}function N(b){if(l.keypress&&l.keypress(b,f)===false)return false;if(f.paused()){if(b.which==100&&b.ctrlKey){for(b=M.length;b--;){var h=M[b];if(4!=h.readyState)try{h.abort()}catch(n){f.error("error in aborting ajax")}}f.resume();return false}}else if(b.which==100&&b.ctrlKey){if(l.exit&&o.get()==="")if(w.size()>1||l.login!==v)f.pop("");else{f.resume();f.echo("")}return false}else if(b.which==
118&&b.ctrlKey){f.oneTime(1,function(){f.attr({scrollTop:f.attr("scrollHeight")})});return true}else if(b.keyCode==9&&b.ctrlKey)f.focus(false);else if(b.which==34)f.scroll(f.height());else if(b.which==33)f.scroll(-f.height());else if(b.which==99&&b.ctrlKey)return false;else f.attr({scrollTop:f.attr("scrollHeight")})}var f=this,E=[],e,q=H.length(),A,l={name:null,prompt:">",history:true,exit:true,clear:true,enabled:true,login:null,onInit:null,onExit:null,keypress:null,keydown:null};if(d){d.width&&f.width(d.width);
d.height&&f.height(d.height);g.extend(l,d)}var G=!l.enabled;if(f.length===0)throw'Sorry, but terminal said that "'+f.selector+'" is not valid selector';if(f.data("terminal")){f.ajaxSend(function(b,h){M.push(h)});return f.data("terminal")}e=g("<div>").addClass("terminal-output").appendTo(f);f.addClass("terminal").append("<div/>");g.extend(f,{clear:function(){e.html("");o.set("");E=[];f.attr({scrollTop:0});return f},paused:function(){return G},pause:function(){if(o){f.disable();o.hide()}return f},resume:function(){if(o){f.enable();
o.show();p(f)}return f},cols:function(){return A},rows:function(){return E.length},history:function(){return o.history().data()},next:function(){if(H.length()==1)return f;else{H.front().disable();var b=H.rotate().enable(),h=b.offset().top-50;g("html,body").animate({scrollTop:h},500);return b}},focus:function(b){f.oneTime(1,function(){if(H.length()==1)b===false?f.disable():f.enable();else if(b===false)f.next();else{H.front().disable();H.set(f);f.enable()}});return f},enable:function(){A===v&&f.resize();
if(G)if(o){o.enable();G=false}return f},disable:function(){if(o){G=true;o.disable()}return f},enabled:function(){return G},signature:function(){var b=f.cols();b=b<15?null:b<35?0:b<55?1:b<64?2:b<75?3:4;return b!==null?ba[b].join("\n")+"\n":""},get_command:function(){return o.get()},insert:function(b){o.insert(b);return f},set_prompt:function(b){j("prompt",b)&&o.prompt(b);return f},set_command:function(b){o.set(b);return f},set_mask:function(b){o.mask(b);return f},get_output:function(){return g.map(E,
function(b,h){return typeof h=="function"?h():h}).get().join("\n")},resize:function(b,h){if(b&&h){f.width(b);f.height(h)}A=c();o.resize(A);var n=e.detach();e.html("");g.each(E,function(s,r){u(typeof r=="function"?r():r)});f.prepend(n);p(f);return f},echo:function(b){E.push(b);return u(typeof b=="function"?b():b)},error:function(b){f.echo(b).addClass("error")},scroll:function(b){b>f.attr("scrollTop")&&b>0&&f.attr("scrollTop",0);var h=f.attr("scrollTop");f.attr("scrollTop",h+b);return f},logout:l.login?
function(){for(;w.size()>1;)w.pop();K();return f}:function(){throw"You don't have login function";},token:l.login?function(){var b=l.name;return g.Storage.get("token"+(b?"_"+b:""))}:null,login_name:l.login?function(){var b=l.name;return g.Storage.get("login_"+(b?"_"+b:""))}:null,name:function(){return l.name},push:function(b,h){if(!h.prompt||j("prompt",h.prompt)){if(typeof b=="string")b=k(h.eval,f);w.push(g.extend({eval:b},h));z()}return f},pop:function(b){b!==v&&m(b);if(w.top().name===l.name){if(l.login){K();
if(typeof l.onExit=="function")l.onExit(f)}}else{b=w.pop();z();if(typeof b.onExit=="function")b.onExit(f)}return f}});var D;switch(typeof a){case "string":D=a;a=k(a,f);break;case "object":a=function(b){return function(h){if(h!=""){h=h.split(/ */);var n=h[0];h=h.slice(1);var s=b[n];typeof s=="function"?s.apply(f,h):f.echo("Command '"+n+"' Not Found")}}}(a)}if(D&&typeof l.login=="string"||D)l.login=function(b){var h=1;return function(n,s,r){f.pause();g.jrpc(D,h++,b,[n,s],function(y){f.resume();!y.error&&
y.result?r(y.result):r(null)},function(y,t){f.resume();f.error("&#91;AJAX&#92; Response: "+t+"\n"+y.responseText)})}}(typeof l.login=="boolean"?"login":l.login);if(j("prompt",l.prompt)){var w=new Z({name:l.name,eval:a,prompt:l.prompt,greetings:l.greetings}),o=f.find(".terminal-output").next().cmd({prompt:l.prompt,history:l.history,width:"100%",keydown:l.keydown?function(b){return l.keydown(b,f)}:null,keypress:N,commands:C});f.livequery(function(){f.resize()});H.append(f);l.enabled===true?f.focus():
f.disable();g(window).resize(f.resize);f.click(function(){f.focus()});f.token&&!f.token()&&f.login_name&&!f.login_name()?F():L();typeof g.fn.init.prototype.mousewheel==="function"&&f.mousewheel(function(b,h){h>0?f.scroll(-40):f.scroll(40);return false},true)}f.data("terminal",f);return f}})(jQuery);
