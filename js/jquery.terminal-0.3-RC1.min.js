...
function(a){function d(){var e=g("<span>").html("&nbsp").appendTo(b);j=Math.floor(b.width()/e.width());e.remove()}var b=this;b.addClass("cmd");b.append('<span class="prompt"></span><span></span><span class="cursor">&nbsp;</span><span></span>');var h=g("<textarea/>").addClass("clipboard").appendTo(b);a.width&&b.width(a.width);var j,p,v=a.mask||false,m="",n=0,B,G=a.enabled,L,z,I=function(){var e=b.find(".cursor");return function(){e.toggleClass("inverted")}}(),C=function(e){function r(o,k){if(k==o.length){y.html(x(o));
l.html("&nbsp;");A.html("")}else if(k===0){y.html("");l.html(x(o.slice(0,1)));A.html(x(o.slice(1)))}else{var c=x(o.slice(0,k));y.html(c);c=o.slice(k,k+1);l.html(c==" "?"&nbsp;":x(c));k==o.lenght-1?A.html(""):A.html(x(o.slice(k+1)))}}function M(o){g.each(o,function(k,c){A.after("<div>"+x(c)+"</div>")})}function E(o){g.each(o,function(k,c){y.before("<div>"+x(c)+"</div>")})}var l=e.find(".cursor"),y=l.prev(),A=l.next();return function(){var o=v?m.replace(/./g,"*"):m;e.find("div").remove();y.html("");
if(o.length>j-p-1){var k;k=o.substring(0,j-p-1);var c=o.substring(j-p-1);k=[k].concat(O(c,j));c=k[0].length;if(n<c){r(k[0],n);M(k.slice(1))}else{var i=k.length;if(n<=c){r(k[0],n);M(k.slice(1))}else{var q=k.slice(-1)[0];o=o.length-n;if(o<q.length){E(k.slice(0,-1));r(q,q.length-o)}else if(i==3){y.before("<div>"+x(k[0])+"</div>");r(k[1],n-c);y.before("<div>"+x(k[2])+"</div>")}else{o=Math.floor(n/j);i=k[o];c=(n-c)%j;q=x(i.slice(0,c));y.html(q);i.slice(c,c+1);c==i.lenght-1?A.html(""):A.html(x(i.slice(c+
1)));E(k.slice(0,o));M(k.slice(o+1))}}}}else if(o===""){y.html("");l.html("&nbsp;");A.html("")}else r(o,n)}}(b),f=function(){var e=b.find(".prompt");return function(){if(typeof B=="string"){p=B.length;e.html(x(B)+"&nbsp;")}else B(function(r){p=r.length;e.html(x(r)+"&nbsp;")});d()}}();g.extend(b,{name:function(e){if(e!==w){L=e;z=new W(e)}else return L},history:function(){return z},set:function(e,r){if(e!==w){m=e;if(!r)n=m.length;C()}},insert:function(e,r){if(n==m.length)m+=e;else m=n===0?e+m:m.slice(0,
n)+e+m.slice(n);r||(n+=e.length);C()},get:function(){return m},commands:function(e){if(e)a.commands=e;else return e},destroy:function(){g(document.documentElement).unbind(".commandline");b.find(".prompt").remove()},prompt:function(e){if(e===w)return B;else{if(typeof e=="string"||typeof e=="function")B=e;else throw"prompt must be a function or string";f()}},position:function(e){if(typeof e=="number"){n=e;C()}else return n},resize:function(){d();C()},enable:function(){if(!this.isenabled()){b.everyTime(500,
"blink",I);G=true}},isenabled:function(){return G},disable:function(){if(this.isenabled()){b.stopTime("blink",I);b.find(".cursor").removeClass("inverted");G=false}},mask:function(e){if(typeof e=="boolean"){v=e;C()}else return v}});b.name(a.name||"");B=a.prompt||">";f();a.enabled===w&&!a.enabled&&b.enable();g(document.documentElement).keypress(function(e){if(G){var r;if(a.keypress)r=a.keypress(e);if(r===w||r)if([38,32,13,40,0,8].has(e.which)&&e.keyCode!=123&&!(e.which==40&&e.shiftKey||e.which==38&&
e.shiftKey))return false;else if(!e.ctrlKey&&!(e.altKey&&e.which==100)){b.insert(String.fromCharCode(e.which));if(e.which==45||e.shiftKey&&e.which==43)return false}}}).keydown(function(e){if(a.keydown&&a.keydown(e)===false)return false;if(G){var r;if(e.keyCode==13){z&&m&&z.append(m);z.last();e=m;b.set("");typeof B=="function"&&f();a.commands&&a.commands(e)}else if(e.which==32)b.insert(" ");else if(e.which==8){if(m!==""&&n>0){m=m.slice(0,n-1)+m.slice(n,m.length);--n;C()}}else if(e.which==9)b.insert("\t");
else if(e.which==46||e.which==68&&e.ctrlKey){if(m!==""&&n<m.length){m=m.slice(0,n)+m.slice(n+1,m.length);C()}return true}else if(z&&e.which==38||e.which==80&&e.ctrlKey)b.set(z.previous());else if(z&&e.which==40||e.which==78&&e.ctrlKey)b.set(z.next());else if(e.which==27)b.set("");else if(e.which==37||e.which==66&&e.ctrlKey)if(e.ctrlKey&&e.which!=66){r=n-1;e=0;for(m[r]==" "&&--r;r>0;--r)if(m[r]==" "&&m[r+1]!=" "){e=r+1;break}b.position(e)}else{if(n>0){--n;C()}}else if(e.which==39||e.which==70&&e.ctrlKey)if(e.ctrlKey&&
...
return b};g.jrpc=function(a,d,b,h,j,p){d=g.json_stringify({jsonrpc:"2.0",method:b,params:h,id:d});return g.ajax({url:a,data:d,success:j,error:p,contentType:"application/json",dataType:"json",beforeSend:function(v){v.setRequestHeader("Foo","bar")},async:true,cache:false,type:"POST"})};K=/ {16}$/;var Y=[["jQuery Terminal","(c) 2011 jcubic"],["JQuery Terminal Emulator v. 0.3-RC.1","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>".replace(/ *<.*>/,"")],["JQuery Terminal Emulator version version 0.3-RC.1",
"Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"],["      _______                 ________                        __","     / / _  /_ ____________ _/__  ___/______________  _____  / /"," __ / / // / // / _  / _/ // / / / _  / _/     / /  \\/ / _ \\/ /","/  / / // / // / ___/ // // / / / ___/ // / / / / /\\  / // / /__","\\___/____ \\\\___/____/_/_\\_ / /_/____/_//_/ /_/_/_/  \\/\\__\\_\\___/","         \\/          /____/                                    ".replace(K,"")+"version 0.3-RC.1",
...
"")+"version 0.3-RC.1","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"]],H=new function(a){var d=a?[a]:[],b=0;g.extend(this,{rotate:function(){if(d.length==1)return d[0];else{if(b==d.length-1)b=0;else++b;return d[b]}},length:function(){return d.length},set:function(h){for(var j=d.length;j--;)if(d[j]===h){b=j;return}this.append(h)},front:function(){return d[b]},append:function(h){d.push(h)}})},Z=[];g.fn.terminal=function(a,d){function b(){if(d.greetings===w)f.signature();else typeof d.greetings==
"string"&&m(d.greetings)}function h(){var c=g("<span>x</span>").appendTo(f),i=Math.floor(f.width()/c.width());c.remove();return i}function j(c,i){if(typeof c=="string")f.echo("["+i+"]: "+c).addClass("error");else{f.echo("["+i+"]: "+c.fileName+": "+c.message).addClass("error");f.pause();g.get(c.fileName,function(q){f.resume();var s=c.lineNumber-1;f.echo("["+c.lineNumber+"]"+q.split("\n")[s]).addClass("error")})}}function p(c,i){try{if(typeof i=="function")i(function(){});else if(typeof i!="string")throw c+
" must be string or function";}catch(q){j(q,c.toUpperCase());return false}return true}function v(c){c.scrollTop(f.attr("scrollHeight"))}function m(c){c=typeof c=="string"?c:String(c);var i;if(c.length>E){c=c.split("\n");i=g("<div></div>");for(var q=c.length,s=0;s<q;++s)if(c[s]===""||c[s]=="\r")i.append("<div>&nbsp;</div>");else if(c[s].length>E){var u=O(c[s],E);g.each(u,function(D,t){g("<div>").html(x(t)).appendTo(i)})}else g("<div>").html(x(c[s])).appendTo(i)}else i=g("<div/>").html(x(c));r.append(i);
i.width("100%");v(f);return i}function n(c,i){var q=1,s=function(u,D){i.pause();g.jrpc(c,q++,u,D,function(t){if(t.error)i.error("[RPC] "+t.error.message);else if(typeof t.result=="string")i.echo(t.result);else if(t.result instanceof Array)i.echo(t.result.join(" "));else if(typeof t.result=="object"){var F="",J;for(J in t.result)if(t.result.hasOwnProperty(J))F+=J+": "+t.result[J]+"\n";i.echo(F)}i.resume()},function(t,F){i.error("[AJAX] "+F+" - Server reponse is: \n"+t.responseText);i.resume()})};return function(u,
D){if(u!==""){var t,F;if(u.match(/[^ ]* /)){u=u.split(/ */);t=u[0];F=u.slice(1)}else{t=u;F=[]}if(!l.login||t=="help")s(t,F);else{var J=D.token();J?s(t,[J].concat(F)):D.error("[AUTH] Access denied (no token)")}}}}function B(c){var i=k.prompt();if(k.mask())c=c.replace(/./g,"*");typeof i=="function"?i(function(q){f.echo(q+" "+c)}):f.echo(i+" "+c)}function G(c){try{var i=o.top();if(c=="exit"&&l.exit)if(o.size()==1)l.login?z():f.echo("You can exit from main interpeter");else f.pop("exit");else{B(c);c==
"clear"&&l.clear?f.clear():i.eval(c,f)}}catch(q){j(q,"USER");throw q;}}function L(){var c=null;k.prompt("login:");l.history&&k.history().disable();k.commands(function(i){try{B(i);if(c){k.mask(false);f.pause();l.login(c,i,function(s){if(s){var u=l.name;u=u?"_"+u:"";g.Storage.set("token"+u,s);g.Storage.set("login"+u,c);k.commands(G);I(true);b()}else{f.error("Wrong password try again");k.prompt("login:");c=null}f.resume();l.history&&k.history().enable()})}else{c=i;k.prompt("password:");k.mask(true)}}catch(q){j(q,
"LOGIN",f);throw q;}})}function z(){var c=l.name;c=c?"_"+c:"";g.Storage.remove("token"+c,null);g.Storage.remove("login"+c,null);l.history&&k.history().disable();L()}function I(){var c=o.top(),i="";if(c.name!==w&&c.name!=="")i+=c.name+"_";i+=M;k.name(i);k.prompt(c.prompt);l.history&&k.history().enable();k.set("");if(typeof c.onStart=="function")c.onStart(f)}function C(c){if(f.paused()){if(c.which==100&&c.ctrlKey){for(c=request.length;c--;){var i=request[c];if(4!=i.readyState)try{i.abort()}catch(q){f.error("error in aborting ajax")}}f.resume();
return false}}else{if(l.keypress&&l.keypress(c)===false)return false;if(c.which==100&&c.ctrlKey&&l.exit&&k.get()===""){l.name==o.top().name&&!l.login?f.echo("you can't exit from top interpreter"):f.pop("");return false}else if(c.charCode==118&&c.ctrlKey){window.setTimeout(function(){f.attr({scrollTop:f.attr("scrollHeight")})},1);return true}else if(c.which==34)f.scroll(f.height());else c.which==33?f.scroll(-f.height()):f.attr({scrollTop:f.attr("scrollHeight")})}}var f=this,e=[],r,M=H.length(),E,l=
{name:null,prompt:">",history:true,exit:true,clear:true,enabled:true,login:null};if(d){d.width&&f.width(d.width);d.height&&f.height(d.height);g.extend(l,d)}var y=!l.enabled;if(f.length===0)throw'Sorry, but terminal said that "'+f.selector+'" is not valid selector';if(f.data("terminal")){f.ajaxSend(function(c,i){Z.push(i)});return f.data("terminal")}r=g("<div>").addClass("terminal-output").appendTo(f);f.addClass("terminal").append("<div/>");g.extend(f,{clear:function(){r.html("");k.set("");e=[];f.attr({scrollTop:0});
...
H.set(f);f.enable()}});return f},enable:function(){E===w&&f.resize();if(y)if(k){k.enable();y=false}return f},disable:function(){if(!y)if(k){y=true;k.disable()}return f},enabled:function(){return y},signature:function(){var c=f.cols();c=c<15?null:c<35?0:c<55?1:c<64?2:c<75?3:4;c!==null&&g.each(Y[c],function(i,q){m(q)})},get_command:function(){return k.get()},insert:function(c){k.insert(c);return f},set_prompt:function(c){p("prompt",c)&&k.prompt(c);return f},set_command:function(c){k.set(c);return f},
set_mask:function(c){k.mask(c);return f},get_output:function(){return e.join("\n")},resize:function(c,i){if(c&&i){f.width(c);f.height(i)}E=h();k.resize();r.html("");b();for(var q=e.length,s=0;s<q;++s)m(e[s]);v(f);return f},echo:function(c){e.push(c);return m(c)},error:function(c){return f.echo(c).addClass("error")},scroll:function(c){c>f.attr("scrollTop")&&c>0&&f.attr("scrollTop",0);var i=f.attr("scrollTop");f.attr("scrollTop",i+c);return f},logout:l.login?function(){for(;o.size()>1;)o.pop();z();
return f}:function(){throw"You don't have login function";},token:l.login?function(){var c=l.name;return g.Storage.get("token"+(c?"_"+c:""))}:null,login_name:l.login?function(){var c=l.name;return g.Storage.get("login_"+(c?"_"+c:""))}:null,name:function(){return l.name},push:function(c,i){if(!i.prompt||p("prompt",i.prompt)){if(typeof c=="string")c=n(i.eval,f);o.push({name:i.name,eval:c,prompt:i.prompt,login:i.login,onStart:i.onStart,onExit:i.onExit});I(true)}return f},pop:function(c){c!==w&&B(c);
if(o.top().name===l.name){if(l.login){z();if(typeof l.onAfterLogout=="function")l.onAfterLogout(f)}}else{c=o.pop();I();if(typeof c.onExit=="function")c.onExit(f)}return f}});var A;switch(typeof a){case "string":A=a;a=n(a,f);break;case "object":a=function(c){return function(i){if(i!=""){i=i.split(/ */);var q=i[0];i=i.slice(1);var s=c[q];typeof s=="function"?s.apply(f,i):f.echo("Command '"+q+"' Not Found")}}}(a)}if(A&&typeof l.login=="string"||A)l.login=function(c){var i=1;return function(q,s,u){f.pause();
g.jrpc(A,i++,c,[q,s],function(D){f.resume();!D.error&&D.result?u(D.result):u(null)},function(D,t){f.resume();f.error("[AJAX] Response: "+t+"\n"+D.responseText)})}}(typeof l.login=="boolean"?"login":l.login);if(p("prompt",l.prompt)){var o=new V({name:l.name,eval:a,prompt:l.prompt,greetings:l.greetings}),k=f.find(".terminal-output").next().cmd({prompt:l.prompt,history:l.history,width:"100%",keydown:l.keydown?function(c){return l.keydown(c,f)}:null,keypress:C,commands:G});f.livequery(function(){f.oneTime(10,
