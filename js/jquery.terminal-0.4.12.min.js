...
var t=g("<textarea/>").addClass("clipboard").appendTo(h);a.width&&h.width(a.width);var v,C,K=a.mask||false,l="",n=0,I,M=a.enabled,V,E,L=h.find(".cursor"),c=function(f){function q(r,u){if(u==r.length){D.html(G(r));L.html("&nbsp;");H.html("")}else if(u===0){D.html("");L.html(G(r.slice(0,1)));H.html(G(r.slice(1)))}else{var o=G(r.slice(0,u));D.html(o);o=r.slice(u,u+1);L.html(o==" "?"&nbsp;":G(o));u==r.lenght-1?H.html(""):H.html(G(r.slice(u+1)))}}function J(r){return"<div>"+G(r)+"</div>"}function Q(r){var u=
H;g.each(r,function(o,y){u=g(J(y)).insertAfter(u).addClass("clear")})}function m(r){g.each(r,function(u,o){D.before(J(o))})}var D=L.prev(),H=L.next();return function(){var r=K?l.replace(/./g,"*"):l,u;f.find("div").remove();D.html("");if(r.length>v-C-1||r.match(/\n/)){var o,y=r.match(/\t/g),p=y?y.length*3:0;if(y)r=r.replace(/\t/g,"\u0000\u0000\u0000\u0000");if(r.match(/\n/)){var b=r.split("\n"),j=v-C-1;for(u=0;u<b.length-1;++u)b[u]+=" ";if(b[0].length>j){o=[b[0].substring(0,j)];o=o.concat(X(b[0].substring(j),
v))}else o=[b[0]];for(u=1;u<b.length;++u)if(b[u].length>v)o=o.concat(X(b[u],v));else o.push(b[u])}else o=e(r);if(y)o=g.map(o,function(k){return k.replace(/\x00\x00\x00\x00/g,"\t")});j=o[0].length;if(n<j){q(o[0],n);Q(o.slice(1))}else if(n==j){D.before(J(o[0]));q(o[1],0);Q(o.slice(2))}else{u=o.length;if(n<j){q(o[0],n);Q(o.slice(1))}else if(n==j){D.before(J(o[0]));q(o[1],0);Q(o.slice(2))}else{y=o.slice(-1)[0];b=r.length-n;r=0;if(b<=y.length){m(o.slice(0,-1));r=y.length==b?0:y.length-b;q(y,r+p)}else if(u==
3){D.before("<div>"+G(o[0])+"</div>");q(o[1],n-j-1);H.after('<div class="clear">'+G(o[2])+"</div>")}else{r=n;for(u=0;u<o.length;++u)if(r>o[u].length)r-=o[u].length;else break;p=o[u];j=u;if(r==p.length){r=0;p=o[++j]}q(p,r);m(o.slice(0,j));Q(o.slice(j+1))}}}}else if(r===""){D.html("");L.html("&nbsp;");H.html("")}else q(r,n)}}(h),N=function(){var f=h.find(".prompt");return function(){if(typeof I=="string"){C=I.replace(Y,"$4").length;f.html(G(I)+"&nbsp;")}else I(function(q){C=q.replace(Y,"$4").length;
f.html(G(q)+"&nbsp;")})}}();g.extend(h,{name:function(f){if(f!==z){V=f;E=new la(f)}else return V},history:function(){return E},set:function(f,q){if(f!==z){l=f;if(!q)n=l.length;c();if(typeof a.onCommandChange=="function")a.onCommandChange(l)}},insert:function(f,q){if(n==l.length)l+=f;else l=n===0?f+l:l.slice(0,n)+f+l.slice(n);q||(n+=f.length);c();if(typeof a.onCommandChange=="function")a.onCommandChange(l)},get:function(){return l},commands:function(f){if(f)a.commands=f;else return f},destroy:function(){g(document.documentElement).unbind(".commandline");
h.find(".prompt").remove()},prompt:function(f){if(f===z)return I;else{if(typeof f=="string"||typeof f=="function")I=f;else throw"prompt must be a function or string";N();c()}},position:function(f){if(typeof f=="number"){n=f<0?0:f>l.length?l.length:f;c()}else return n},show:function(){var f=h.show;return function(){f.apply(h,[]);c();N()}}(),resize:function(f){if(f)v=f;else{f=h.width();var q=L.innerWidth();v=Math.floor(f/q)}c()},enable:function(){if(!M){h.everyTime(500,"blink",d);M=true}},isenabled:function(){return M},
disable:function(){if(M){h.stopTime("blink",d);h.find(".cursor").removeClass("inverted");M=false}},mask:function(f){if(typeof f=="boolean"){K=f;c()}else return K}});h.name(a.name||"");I=a.prompt||">";N();if(a.enabled===z||a.enabled===true)h.enable();g(g.browser.msie?document.documentElement:window).keypress(function(f){var q;if(f.ctrlKey&&f.which==99)return true;if(a.keypress)q=a.keypress(f);if(q===z||q){if(M)if([38,32,13,0,8].has(f.which)&&f.keyCode!=123&&!(f.which==38&&f.shiftKey))return false;
else if(!f.ctrlKey&&!(f.altKey&&f.which==100)){h.insert(String.fromCharCode(f.which));return false}else f.altKey&&h.insert(String.fromCharCode(f.which))}else return q}).keydown(function(f){if(M){if(a.keydown&&a.keydown(f)===false)return false;var q;if(f.altKey){if(f.which==68){h.set(l.slice(0,n)+l.slice(n).replace(/[^ ]+ |[^ ]+$/,""),true);return false}return true}else if(f.keyCode==13){if(E&&l&&(a.historyFilter&&a.historyFilter(l)||!a.historyFilter))E.append(l);E.last();f=l;h.set("");a.commands&&
a.commands(f);typeof I=="function"&&N()}else if(f.which==32)h.insert(" ");else if(f.which==8){if(l!==""&&n>0){l=l.slice(0,n-1)+l.slice(n,l.length);--n;c()}}else if(f.which==9&&!(f.ctrlKey||f.altKey))h.insert("\t");else if(f.which==46){if(l!==""&&n<l.length){l=l.slice(0,n)+l.slice(n+1,l.length);c()}return true}else if(E&&f.which==38||f.which==80&&f.ctrlKey)h.set(E.previous());else if(E&&f.which==40||f.which==78&&f.ctrlKey)h.set(E.next());else if(f.which==37||f.which==66&&f.ctrlKey)if(f.ctrlKey&&f.which!=
66){q=n-1;f=0;for(l[q]==" "&&--q;q>0;--q)if(l[q]==" "&&l[q+1]!=" "){f=q+1;break}else if(l[q]=="\n"&&l[q+1]!="\n"){f=q;break}h.position(f)}else{if(n>0){--n;c()}}else if(f.which==39||f.which==70&&f.ctrlKey)if(f.ctrlKey&&f.which!=70){l[n]==" "&&++n;f=l.slice(n).match(/\S[\n\s]{2,}|[\n\s]+\S?/);if(!f||f[0].match(/^\s+$/))n=l.length;else if(f[0][0]!=" ")n+=f.index+1;else{n+=f.index+f[0].length-1;f[0][f[0].length-1]!=" "&&--n}c()}else{if(n<l.length){++n;c()}}else if(f.which==123)return true;else if(f.which==
36)h.position(0);else if(f.which==35)h.position(l.length);else if(f.ctrlKey||f.metaKey)if(f.shiftKey){if(f.which==84)return true}else if(f.which==65)h.position(0);else if(f.which==69)h.position(l.length);else if(f.which==88||f.which==67||f.which==87||f.which==84)return true;else if(f.which==86){i();return true}else if(f.which==75)if(n===0)h.set("");else n!=l.length&&h.set(l.slice(0,n));else if(f.which==85){h.set(l.slice(n,l.length));h.position(0)}else{if(f.which==17)return true}else return true;return false}});
...
b.toUpperCase());return false}return true}function v(){var b=c.prop?c.prop("scrollHeight"):c.attr("scrollHeight");c.scrollTop(b)}function C(b){b=typeof b=="string"?b:String(b);var j,k,w;if(b.length>J){j=J;k=b.split(/\n/g);w=/(\[\[[bius]*;[^;]*;[^\]]*\][^\]\[]*\]?)/g;var s=/(\[\[[bius]*;[^;]*;[^\]]*\])/,A=/\[\[[bius]*;?[^;]*;?[^\]]*\]?$/,x=false,F=false,B="";b=[];for(var T=0,pa=k.length;T<pa;++T){if(B!=="")if(k[T]===""){b.push(B+"]");continue}else{k[T]=B+k[T];B=""}else if(k[T]===""){b.push("");continue}for(var W=
k[T],ca=0,Z=0,R=0,da=W.length;R<da;++R){if(W[R]=="["&&W[R+1]=="[")x=true;else if(x&&W[R]=="]")if(F)F=x=false;else F=true;else if(x&&F||!x)++Z;if(Z==j||R==da-1){var P=W.substring(ca,R+1);if(B){P=B+P;if(P.match("]"))B=""}ca=R+1;Z=0;var S=P.match(w);if(S){S=S[S.length-1];if(S[S.length-1]!="]"){B=S.match(s)[1];P+="]"}else if(P.match(A)){P=P.replace(A,"");B=S.match(s)[1]}}b.push(P)}}}j=g("<div></div>");k=0;for(w=b.length;k<w;++k)b[k]===""||b[k]=="\r"?j.append("<div>&nbsp;</div>"):g("<div/>").html(G(b[k])).appendTo(j)}else j=
g("<div/>").html(G(b));f.append(j);j.width("100%");v();return j}function K(){if(d.greetings===z)c.echo(c.signature);else d.greetings&&c.echo(d.greetings)}function l(b,j){var k=1,w=function(s,A){j.pause();g.jrpc(b,k++,s,A,function(x){if(x.error)j.error("&#91;RPC&#93; "+x.error.message);else if(typeof x.result=="string")j.echo(x.result);else if(x.result instanceof Array)j.echo(x.result.join(" "));else if(typeof x.result=="object"){var F="",B;for(B in x.result)if(x.result.hasOwnProperty(B))F+=B+": "+
x.result[B]+"\n";j.echo(F)}j.resume()},function(x,F){j.error("&#91;AJAX&#93; "+F+" - Server reponse is: \n"+x.responseText);j.resume()})};return function(s,A){if(s!==""){var x,F;if(s.match(/[^ ]* /)){s=s.split(/ +/);x=s[0];F=s.slice(1)}else{x=s;F=[]}if(!m.login||x=="help")w(x,F);else{var B=A.token();B?w(x,[B].concat(F)):A.error("&#91;AUTH&#93; Access denied (no token)")}}}}function n(b){var j=p.prompt();if(p.mask())b=b.replace(/./g,"*");typeof j=="function"?j(function(k){c.echo(k+" "+b)}):c.echo(j+
" "+b)}function I(b){try{var j=y.top();if(b=="exit"&&m.exit)if(y.size()==1)m.login?V():c.echo("You can exit from main interpeter");else c.pop("exit");else{n(b);b=="clear"&&m.clear?c.clear():j.eval(b,c)}}catch(k){h(k,"USER");c.resume();throw k;}}function M(){var b=null;p.prompt("login:");m.history&&p.history().disable();p.commands(function(j){try{n(j);if(b){p.mask(false);c.pause();if(typeof m.login!="function")throw"Value of login property must be a function";m.login(b,j,function(w){if(w){var s=m.name;
s=s?"_"+s:"";g.Storage.set("token"+s,w);g.Storage.set("login"+s,b);p.commands(I);E();K()}else{c.error("Wrong password try again");p.prompt("login:");b=null}c.resume();m.history&&p.history().enable()})}else{b=j;p.prompt("password:");p.mask(true)}}catch(k){h(k,"LOGIN",c);throw k;}})}function V(){var b=m.name;b=b?"_"+b:"";g.Storage.remove("token"+b,null);g.Storage.remove("login"+b,null);m.history&&p.history().disable();M()}function E(){var b=y.top(),j="";if(b.name!==z&&b.name!=="")j+=b.name+"_";j+=q;
p.name(j);p.prompt(b.prompt);m.history&&p.history().enable();p.set("");if(typeof b.onStart=="function")b.onStart(c)}function L(b){c.oneTime(5,function(){if(r!=e()){c.resize();r=e()}});if(!c.paused()){if(m.keydown&&m.keydown(b,c)===false)return false;if(b.which!=9)H=0;if(b.which==68&&b.ctrlKey){if(m.exit)if(p.get()==="")if(y.size()>1||m.login!==z)c.pop("");else{c.resume();c.echo("")}else c.set_command("");b.preventDefault()}else if(m.tabcompletion&&b.which==9){++H;b=p.get();if(!b.match(" ")){for(var j=
RegExp("^"+b),k=y.top().command_list,w=[],s=k.length;s--;)j.test(k[s])&&w.push(k[s]);if(w.length==1)c.set_command(w[0]);else if(w.length>1)if(H>=2){n(b);c.echo(w.join("\t"));H=0}}return false}else if(b.which==86&&b.ctrlKey){c.oneTime(1,function(){v()});return true}else if(b.which==9&&b.ctrlKey){O.length()>1&&c.focus(false);b.preventDefault()}else if(b.which==34)c.scroll(c.height());else b.which==33?c.scroll(-c.height()):c.attr({scrollTop:c.attr("scrollHeight")})}}var c=this,N=[],f,q=O.length(),J,
Q=[],m={name:"",prompt:">",history:true,exit:true,clear:true,enabled:true,login:null,tabcompletion:false,onInit:null,onExit:null,keypress:null,keydown:null};if(d){d.width&&c.width(d.width);d.height&&c.height(d.height);g.extend(m,d)}var D=!m.enabled;if(c.length===0)throw'Sorry, but terminal said that "'+c.selector+'" is not valid selector!';c.ajaxSend(function(b,j){oa.push(j)});if(c.data("terminal"))return c.data("terminal");f=g("<div>").addClass("terminal-output").appendTo(c);c.addClass("terminal").append("<div/>");
g.extend(c,{clear:function(){f.html("");p.set("");N=[];c.attr({scrollTop:0});return c},paused:function(){return D},pause:function(){if(p){c.disable();p.hide()}return c},resume:function(){if(p){c.enable();p.show();v()}return c},cols:function(){return J},rows:function(){return N.length},history:function(){return p.history()},next:function(){if(O.length()==1)return c;else{var b=c.offset().top;c.height();c.scrollTop();var j=c,k=g(window).scrollTop(),w=k+g(window).height(),s=g(j).offset().top;if(s+g(j).height()>=
k&&s<=w){O.front().disable();b=O.rotate().enable();j=b.offset().top-50;g("html,body").animate({scrollTop:j},500);return b}else{c.enable();g("html,body").animate({scrollTop:b-50},500);return c}}},focus:function(b){c.oneTime(1,function(){if(O.length()==1)b===false?c.disable():c.enable();else if(b===false)c.next();else{O.front().disable();O.set(c);c.enable()}});return c},enable:function(){J===z&&c.resize();if(D)if(p){p.enable();D=false}return c},disable:function(){if(p){D=true;p.disable()}return c},
...
"function"?j():j}).get().join("\n")},resize:function(b,j){if(b&&j){c.width(b);c.height(j)}J=i();p.resize(J);var k=f.detach();f.html("");g.each(N,function(w,s){C(typeof s=="function"?s():s)});c.prepend(k);v();return c},echo:function(b){N.push(b);return C(typeof b=="function"?b():b)},error:function(b){c.echo("[[;#f00;]"+b.replace(/\[/g,"&#91;").replace(/\]/g,"&#93;")+"]")},scroll:function(b){var j;if(c.prop){b>c.prop("scrollTop")&&b>0&&c.prop("scrollTop",0);j=c.prop("scrollTop");c.prop("scrollTop",
j+b)}else{b>c.attr("scrollTop")&&b>0&&c.attr("scrollTop",0);j=c.attr("scrollTop");c.attr("scrollTop",j+b)}return c},logout:m.login?function(){for(;y.size()>1;)y.pop();V();return c}:function(){throw"You don't have login function";},token:m.login?function(){var b=m.name;return g.Storage.get("token"+(b?"_"+b:""))}:null,login_name:m.login?function(){var b=m.name;return g.Storage.get("login"+(b?"_"+b:""))}:null,name:function(){return m.name},push:function(b,j){if(!j.prompt||t("prompt",j.prompt)){if(typeof b==
"string")b=l(j.eval,c);y.push(g.extend({eval:b},j));E()}return c},pop:function(b){b!==z&&n(b);if(y.top().name===m.name){if(m.login){V();if(typeof m.onExit=="function")m.onExit(c)}}else{b=y.pop();E();if(typeof b.onExit=="function")b.onExit(c)}return c}});var H=0,r=e(),u;if(typeof m.onInit=="function")m.onInit(c);if(a.constructor==String){u=a;a=l(a,c)}else if(a.constructor==Array)throw"You can't use array as eval";else if(typeof a=="object"){for(var o in a)Q.push(o);a=function b(j){return function(k){if(k!==
""){k=k.split(/ +/);var w=k[0],s=k.slice(1);k=j[w];var A=typeof k;if(A=="function")k.apply(c,s);else if(A=="object"||A=="string"){s=[];if(A=="object"){for(var x in k)s.push(x);k=b(k)}c.push(k,{prompt:w+">",name:w,command_list:s})}else c.error("Command '"+w+"' Not Found")}}}(a)}else if(typeof a!="function")throw'Unknow object "'+String(a)+'" passed as eval';if(u&&(typeof m.login=="string"||m.login))m.login=function(b){var j=1;return function(k,w,s){c.pause();g.jrpc(u,j++,b,[k,w],function(A){c.resume();
!A.error&&A.result?s(A.result):s(null)},function(A,x){c.resume();c.error("&#91;AJAX&#92; Response: "+x+"\n"+A.responseText)})}}(typeof m.login=="boolean"?"login":m.login);if(t("prompt",m.prompt)){var y=new ka({name:m.name,eval:a,prompt:m.prompt,command_list:Q,greetings:m.greetings}),p=c.find(".terminal-output").next().cmd({prompt:m.prompt,history:m.history,width:"100%",keydown:L,keypress:m.keypress?function(b){return m.keypress(b,c)}:null,onCommandChange:function(b){if(typeof m.onCommandChange=="function")m.onCommandChange(b);
v()},commands:I});c.livequery(function(){c.resize()});O.append(c);m.enabled===true?c.focus():c.disable();g(window).resize(c.resize);c.click(function(){c.focus()});if(c.token&&!c.token()&&c.login_name&&!c.login_name())M();else{E();K()}typeof g.fn.init.prototype.mousewheel==="function"&&c.mousewheel(function(b,j){j>0?c.scroll(-40):c.scroll(40);return false},true)}c.data("terminal",c);return c}})(jQuery);
