...
"string"){c=c.replace(/&(?!#[0-9]+;|[a-zA-Z]+;)/g,"&amp;");c=c.replace(/</g,"&lt;").replace(/>/g,"&gt;");c=c.replace(/\n/g,"<br/>");c=c.replace(/ /g,"&nbsp;");c=c.replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;");var e=c.split(ma);if(e.length>1)c=m.map(e,function(f){return f===""?f:f[0]=="["?f.replace(Y,function(h,k,t,y,n){if(n==="")return"<span>&nbsp;</span>";h="";if(k.indexOf("b")!=-1)h+="font-weight:bold;";var D="text-decoration:";if(k.indexOf("u")!=-1)D+="underline ";if(k.indexOf("s")!=-1)D+="line-through";
if(k.indexOf("s")!=-1||k.indexOf("u")!=-1)h+=D+";";if(k.indexOf("i")!=-1)h+="font-style:italic; ";if(t.match(ca))h+="color:"+t+";";if(y.match(ca))h+="background-color:"+y;return c='<span style="'+h+'">'+n+"</span>"}):"<span>"+f+"</span>"}).join("");return c}else return""}function da(c){var e=c instanceof Array?c:c?[c]:[],f=0;m.extend(this,{left:function(){if(f===0)f=e.length-1;else--f;return e[f]},right:function(){if(f==e.length-1)f=0;else++f;return e[f]},current:function(){return e[f]},data:function(){return e},
length:function(){return e.length},reset:function(){f=0},append:function(h){e.push(h);this.reset()}})}function na(c){var e=c?[c]:[];m.extend(this,{size:function(){return e.length},pop:function(){if(e.length===0)return null;else{var f=e[e.length-1];e=e.slice(0,e.length-1);return f}},push:function(f){e=e.concat([f]);return f},top:function(){return e.length>0?e[e.length-1]:null}})}function oa(c){var e=true;if(typeof c==="string"&&c!=="")c+="_";var f=m.Storage.get(c+"commands"),h=new da(f?eval("("+f+
")"):[""]);m.extend(this,{append:function(k){if(e&&h.current()!=k){h.append(k);m.Storage.set(c+"commands",m.json_stringify(h.data()))}},data:function(){return h.data()},next:function(){return h.right()},last:function(){h.reset()},previous:function(){return h.left()},clear:function(){h=new da;m.Storage.remove(c+"commands")},enable:function(){e=true},disable:function(){e=false}})}var T=typeof window.localStorage!=="undefined";m.extend({Storage:{set:T?ha:ba,get:T?ia:ja,remove:T?ka:la}});jQuery.fn.extend({everyTime:function(c,
e,f,h,k){return this.each(function(){jQuery.timer.add(this,c,e,f,h,k)})},oneTime:function(c,e,f){return this.each(function(){jQuery.timer.add(this,c,e,f,1)})},stopTime:function(c,e){return this.each(function(){jQuery.timer.remove(this,c,e)})}});jQuery.extend({timer:{guid:1,global:{},regex:/^([0-9]+)\s*(.*s)?$/,powers:{ms:1,cs:10,ds:100,s:1E3,das:1E4,hs:1E5,ks:1E6},timeParse:function(c){if(c==E||c===null)return null;var e=this.regex.exec(jQuery.trim(c.toString()));return e[2]?parseInt(e[1],10)*(this.powers[e[2]]||
1):c},add:function(c,e,f,h,k,t){var y=0;if(jQuery.isFunction(f)){k||(k=h);h=f;f=e}e=jQuery.timer.timeParse(e);if(!(typeof e!="number"||isNaN(e)||e<=0)){if(k&&k.constructor!=Number){t=!!k;k=0}k=k||0;t=t||false;if(!c.$timers)c.$timers={};c.$timers[f]||(c.$timers[f]={});h.$timerID=h.$timerID||this.guid++;var n=function(){if(!(t&&this.inProgress)){this.inProgress=true;if(++y>k&&k!==0||h.call(c,y)===false)jQuery.timer.remove(c,f,h);this.inProgress=false}};n.$timerID=h.$timerID;c.$timers[f][h.$timerID]||
(c.$timers[f][h.$timerID]=window.setInterval(n,e));this.global[f]||(this.global[f]=[]);this.global[f].push(c)}},remove:function(c,e,f){var h=c.$timers,k;if(h){if(e){if(h[e]){if(f){if(f.$timerID){window.clearInterval(h[e][f.$timerID]);delete h[e][f.$timerID]}}else for(f in h[e]){window.clearInterval(h[e][f]);delete h[e][f]}for(k in h[e])break;if(!k){k=null;delete h[e]}}}else for(e in h)this.remove(c,e,f);for(k in h)break;if(!k)c.$timers=null}}}});if(jQuery.browser.msie)jQuery(window).one("unload",
