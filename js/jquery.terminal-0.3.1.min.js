...
d+1);c+=g.json_stringify(a[i-1],d+1)+"]"}else{c+="{";for(i in a)if(a.hasOwnProperty(i))c+='"'+i+'":'+g.json_stringify(a[i],d+1);c+="}"}break;case "string":i=a;var p={"\\\\":"\\\\",'"':'\\"',"/":"\\/","\\n":"\\n","\\r":"\\r","\\t":"\\t"};for(j in p)if(p.hasOwnProperty(j))i=i.replace(RegExp(j,"g"),p[j]);c+='"'+i+'"';break;case "number":c+=String(a)}c+=d>1?",":"";if(d==1)c=c.replace(/,([\]}])/g,"$1");return c.replace(/([\[{]),/g,"$1")};g.fn.cmd=function(a){function d(e){var r=e.substring(0,j-p-1);e=
e.substring(j-p-1);return[r].concat(P(e,j))}var c=this;c.addClass("cmd");c.append('<span class="prompt"></span><span></span><span class="cursor">&nbsp;</span><span></span>');var i=g("<textarea/>").addClass("clipboard").appendTo(c);a.width&&c.width(a.width);var j,p,u=a.mask||false,k="",m=0,C,F=a.enabled,K,z,L=function(){var e=c.find(".cursor");return function(){e.toggleClass("inverted")}}(),N=c.find(".cursor"),f=function(e){function r(b,h){if(h==b.length){w.html(x(b));D.html("&nbsp;");o.html("")}else if(h===
...
j-p-1){var h=d(b),n=h[0].length;if(m<n){r(h[0],m);l(h.slice(1))}else if(m==n){w.before(A(h[0]));r(h[1],0);l(h.slice(2))}else{var s=h.length;if(m<n){r(h[0],m);l(h.slice(1))}else if(m==n){w.before(A(h[0]));r(h[1],0);l(h.slice(2))}else{var q=h.slice(-1)[0];b=b.length-m;if(b<=q.length){G(h.slice(0,-1));n=q.length==b?0:q.length-b;r(q,n)}else if(s==3){w.before("<div>"+x(h[0])+"</div>");r(h[1],m-n-1);o.after("<div>"+x(h[2])+"</div>")}else{q=Math.floor((m+p)/j);s=h[q];n=function(y){for(var t=0,B=y.length;B--;)t+=
y[B].length;return t}(h.slice(0,q));n=m-n;if(n==j){n=0;s=h[++q]}x(s.slice(0,n));r(s,n);G(h.slice(0,q));l(h.slice(q+1))}}}}else if(b===""){w.html("");D.html("&nbsp;");o.html("")}else r(b,m)}}(c),E=function(){var e=c.find(".prompt");return function(){if(typeof C=="string"){p=C.length;e.html(x(C)+"&nbsp;")}else C(function(r){p=r.length;e.html(x(r)+"&nbsp;")})}}();g.extend(c,{name:function(e){if(e!==v){K=e;z=new $(e)}else return K},history:function(){return z},set:function(e,r){if(e!==v){k=e;if(!r)m=
k.length;f()}},insert:function(e,r){if(m==k.length)k+=e;else k=m===0?e+k:k.slice(0,m)+e+k.slice(m);r||(m+=e.length);f()},get:function(){return k},commands:function(e){if(e)a.commands=e;else return e},destroy:function(){g(document.documentElement).unbind(".commandline");c.find(".prompt").remove()},prompt:function(e){if(e===v)return C;else{if(typeof e=="string"||typeof e=="function")C=e;else throw"prompt must be a function or string";E()}},position:function(e){if(typeof e=="number"){m=e<0?0:e>k.length?
k.length:e;f()}else return m},resize:function(e){if(e)j=e;else{e=c.width();var r=N.innerWidth();j=Math.floor(e/r)}f()},enable:function(){if(!this.isenabled()){c.everyTime(500,"blink",L);F=true}},isenabled:function(){return F},disable:function(){if(this.isenabled()){c.stopTime("blink",L);c.find(".cursor").removeClass("inverted");F=false}},mask:function(e){if(typeof e=="boolean"){u=e;f()}else return u}});c.name(a.name||"");C=a.prompt||">";E();if(a.enabled===v||a.enabled===true)c.enable();g(document.documentElement).keypress(function(e){if(F){var r;
if(a.keypress)r=a.keypress(e);if(r===v||r)if([38,32,13,40,0,8].has(e.which)&&e.keyCode!=123&&!(e.which==40&&e.shiftKey||e.which==38&&e.shiftKey))return false;else if(!e.ctrlKey&&!(e.altKey&&e.which==100)){c.insert(String.fromCharCode(e.which));if(e.which==45||e.shiftKey&&e.which==43)return false}}if(e.which==100&&e.ctrlKey)return false}).keydown(function(e){if(a.keydown&&a.keydown(e)===false)return false;if(F){var r;if(e.keyCode==13){z&&k&&z.append(k);z.last();e=k;c.set("");typeof C=="function"&&
E();a.commands&&a.commands(e)}else if(e.which==32)c.insert(" ");else if(e.which==8){if(k!==""&&m>0){k=k.slice(0,m-1)+k.slice(m,k.length);--m;f()}}else if(e.which==9&&!(e.ctrlKey||e.altKey))c.insert("\t");else if(e.which==46||e.which==68&&e.ctrlKey){if(k!==""&&m<k.length){k=k.slice(0,m)+k.slice(m+1,k.length);f()}return true}else if(z&&e.which==38||e.which==80&&e.ctrlKey)c.set(z.previous());else if(z&&e.which==40||e.which==78&&e.ctrlKey)c.set(z.next());else if(e.which==27)c.set("");else if(e.which==
37||e.which==66&&e.ctrlKey)if(e.ctrlKey&&e.which!=66){r=m-1;e=0;for(k[r]==" "&&--r;r>0;--r)if(k[r]==" "&&k[r+1]!=" "){e=r+1;break}c.position(e)}else{if(m>0){--m;f()}}else if(e.which==39||e.which==70&&e.ctrlKey)if(e.ctrlKey&&e.which!=70){k[m]==" "&&++m;e=k.slice(m).match(/[^ ] {2,}| +[^ ]?/);if(!e||e[0].match(/^ +$/))m=k.length;else if(e[0][0]!=" ")m+=e.index+1;else{m+=e.index+e[0].length-1;e[0][e[0].length-1]!=" "&&--m}f()}else{if(m<k.length){++m;f()}}else if(e.which==123)return true;else if(e.which==
36)c.position(0);else if(e.which==35)c.position(k.length);else if(e.metaKey)if(e.shiftKey){if(e.which==84)return true}else{if(!e.altKey)if(e.which==65)c.position(0);else if(e.which==69)c.position(k.length);else if(e.which==88||e.which==67||e.which==87||e.which==84)return true;else if(e.which==86){i.focus();c.oneTime(1,function(){c.insert(i.val());i.blur();i.val("")});return true}else if(e.which==75)if(m===0)c.set("");else m!=k.length&&c.set(k.slice(0,m));else if(e.which==17)return true}else if(e.altKey)e.which==
...
var ba=[["jQuery Terminal","(c) 2011 jcubic"],["JQuery Terminal Emulator v. 0.3.1","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>".replace(/ *<.*>/,"")],["JQuery Terminal Emulator version version 0.3.1","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"],["      _______                 ________                        __","     / / _  /_ ____________ _/__  ___/______________  _____  / /"," __ / / // / // / _  / _/ // / / / _  / _/     / /  \\/ / _ \\/ /","/  / / // / // / ___/ // // / / / ___/ // / / / / /\\  / // / /__",
"\\___/____ \\\\__/____/_/ \\__ / /_/____/_//_/ /_/ /_/  \\/\\__\\_\\___/","         \\/          /____/                                   ".replace(J,"")+"version 0.3.1","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"],["      __ _____                     ________                              __","     / // _  /__ __ _____ ___ __ _/__  ___/__ ___ ______ __ __  __ ___  / /"," __ / // // // // // _  // _// // / / // _  // _//     // //  \\/ // _ \\/ /","/  / // // // // // ___// / / // / / // ___// / / / / // // /\\  // // / /__",
"\\___//____ \\\\___//____//_/ _\\_  / /_//____//_/ /_/ /_//_//_/ /_/ \\__\\_\\___/","          \\/              /____/                                          ".replace(J,"")+"version 0.3.1","Copyright (c) 2011 Jakub Jankiewicz <http://jcubic.pl>"]],H=new function(a){var d=a?[a]:[],c=0;g.extend(this,{rotate:function(){if(d.length==1)return d[0];else{if(c==d.length-1)c=0;else++c;return d[c]}},length:function(){return d.length},set:function(i){for(var j=d.length;j--;)if(d[j]===i){c=j;return}this.append(i)},
...
else if(typeof h!="string")throw b+" must be string or function";}catch(n){i(n,b.toUpperCase());return false}return true}function p(b){b.scrollTop(f.attr("scrollHeight"))}function u(b){b=typeof b=="string"?b:String(b);var h;if(b.length>A){b=b.split("\n");h=g("<div></div>");for(var n=b.length,s=0;s<n;++s)if(b[s]===""||b[s]=="\r")h.append("<div>&nbsp;</div>");else if(b[s].length>A){var q=P(b[s],A);g.each(q,function(y,t){g("<div/>").html(x(t)).appendTo(h)})}else g("<div/>").html(x(b[s])).appendTo(h)}else h=
g("<div/>").html(x(b));e.append(h);h.width("100%");p(f);return h}function k(b,h){var n=1,s=function(q,y){h.pause();g.jrpc(b,n++,q,y,function(t){if(t.error)h.error("&#91;RPC&#93; "+t.error.message);else if(typeof t.result=="string")h.echo(t.result);else if(t.result instanceof Array)h.echo(t.result.join(" "));else if(typeof t.result=="object"){var B="",I;for(I in t.result)if(t.result.hasOwnProperty(I))B+=I+": "+t.result[I]+"\n";h.echo(B)}h.resume()},function(t,B){h.error("&#91;AJAX&#93; "+B+" - Server reponse is: \n"+
t.responseText);h.resume()})};return function(q,y){if(q!==""){var t,B;if(q.match(/[^ ]* /)){q=q.split(/ */);t=q[0];B=q.slice(1)}else{t=q;B=[]}if(!l.login||t=="help")s(t,B);else{var I=y.token();I?s(t,[I].concat(B)):y.error("&#91;AUTH&#93; Access denied (no token)")}}}}function m(b){var h=o.prompt();if(o.mask())b=b.replace(/./g,"*");typeof h=="function"?h(function(n){f.echo(n+" "+b)}):f.echo(h+" "+b)}function C(b){try{var h=w.top();if(b=="exit"&&l.exit)if(w.size()==1)l.login?K():f.echo("You can exit from main interpeter");
else f.pop("exit");else{m(b);b=="clear"&&l.clear?f.clear():h.eval(b,f)}}catch(n){i(n,"USER");throw n;}}function F(){var b=null;o.prompt("login:");l.history&&o.history().disable();o.commands(function(h){try{m(h);if(b){o.mask(false);f.pause();l.login(b,h,function(s){if(s){var q=l.name;q=q?"_"+q:"";g.Storage.set("token"+q,s);g.Storage.set("login"+q,b);o.commands(C);L()}else{f.error("Wrong password try again");o.prompt("login:");b=null}f.resume();l.history&&o.history().enable()})}else{b=h;o.prompt("password:");
o.mask(true)}}catch(n){i(n,"LOGIN",f);throw n;}})}function K(){var b=l.name;b=b?"_"+b:"";g.Storage.remove("token"+b,null);g.Storage.remove("login"+b,null);l.history&&o.history().disable();F()}function z(){var b=w.top(),h="";if(b.name!==v&&b.name!=="")h+=b.name+"_";h+=r;o.name(h);o.prompt(b.prompt);l.history&&o.history().enable();o.set("");if(typeof b.onStart=="function")b.onStart(f)}function L(){z();if(d.greetings===v)f.echo(f.signature);else d.greetings&&f.echo(d.greetings);if(typeof l.onInit=="function")l.onInit(f)}
function N(b){if(f.paused()){if(b.which==100&&b.ctrlKey){for(b=M.length;b--;){var h=M[b];if(4!=h.readyState)try{h.abort()}catch(n){f.error("error in aborting ajax")}}f.resume();return false}}else{if(l.keypress&&l.keypress(b)===false)return false;if(b.which==100&&b.ctrlKey){if(l.exit&&o.get()==="")if(w.size()>1||l.login!==v)f.pop("");else{f.resume();f.echo("")}return false}else if(b.which==118&&b.ctrlKey){f.oneTime(1,function(){f.attr({scrollTop:f.attr("scrollHeight")})});return true}else if(b.keyCode==
9&&b.ctrlKey)f.focus(false);else if(b.which==34)f.scroll(f.height());else if(b.which==33)f.scroll(-f.height());else if(b.which==99&&b.ctrlKey)return false;else f.attr({scrollTop:f.attr("scrollHeight")})}}var f=this,E=[],e,r=H.length(),A,l={name:null,prompt:">",history:true,exit:true,clear:true,enabled:true,login:null,onInit:null,onExit:null,keypress:null,keydown:null};if(d){d.width&&f.width(d.width);d.height&&f.height(d.height);g.extend(l,d)}var G=!l.enabled;if(f.length===0)throw'Sorry, but terminal said that "'+
f.selector+'" is not valid selector';if(f.data("terminal")){f.ajaxSend(function(b,h){M.push(h)});return f.data("terminal")}e=g("<div>").addClass("terminal-output").appendTo(f);f.addClass("terminal").append("<div/>");g.extend(f,{clear:function(){e.html("");o.set("");E=[];f.attr({scrollTop:0});return f},paused:function(){return G},pause:function(){if(o){f.disable();o.hide()}return f},resume:function(){if(o){f.enable();o.show();p(f)}return f},cols:function(){return A},rows:function(){return E.length},
history:function(){return o.history().data()},next:function(){if(H.length()==1)return f;else{H.front().disable();var b=H.rotate().enable(),h=b.offset().top-50;g("html,body").animate({scrollTop:h},500);return b}},focus:function(b){f.oneTime(1,function(){if(H.length()==1)b===false?f.disable():f.enable();else if(b===false)f.next();else{H.front().disable();H.set(f);f.enable()}});return f},enable:function(){A===v&&f.resize();if(G)if(o){o.enable();G=false}return f},disable:function(){if(o){G=true;o.disable()}return f},
enabled:function(){return G},signature:function(){var b=f.cols();b=b<15?null:b<35?0:b<55?1:b<64?2:b<75?3:4;return b!==null?ba[b].join("\n")+"\n":""},get_command:function(){return o.get()},insert:function(b){o.insert(b);return f},set_prompt:function(b){j("prompt",b)&&o.prompt(b);return f},set_command:function(b){o.set(b);return f},set_mask:function(b){o.mask(b);return f},get_output:function(){return g.map(E,function(b,h){return typeof h=="function"?h():h}).get().join("\n")},resize:function(b,h){if(b&&
h){f.width(b);f.height(h)}A=c();o.resize(A);var n=e.detach();e.html("");g.each(E,function(s,q){u(typeof q=="function"?q():q)});f.prepend(n);p(f);return f},echo:function(b){E.push(b);return u(typeof b=="function"?b():b)},error:function(b){f.echo(b).addClass("error")},scroll:function(b){b>f.attr("scrollTop")&&b>0&&f.attr("scrollTop",0);var h=f.attr("scrollTop");f.attr("scrollTop",h+b);return f},logout:l.login?function(){for(;w.size()>1;)w.pop();K();return f}:function(){throw"You don't have login function";
