...
"(reverse-i-search)`"+F+"': ";H()}function i(c){var q=D.data(),P=RegExp("^"+F),w=q.length;if(c&&Q>0)w-=Q;for(c=w;c--;)if(P.test(q[c])){Q=q.length-c;o=0;p.set(q[c],true);x();break}}function m(c){var q=c.substring(0,A-K-1);c=c.substring(A-K-1);return[q].concat(X(c,A))}function y(){J.focus();p.oneTime(1,function(){p.insert(J.val());J.blur().val("")})}function C(c){if(d.keydown&&d.keydown(c)===false)return false;if(L){var q;if(G&&(c.which==35||c.which==36||c.which==37||c.which==38||c.which==39||c.which==
...
return function(){if(typeof z=="string"){K=z.replace(Y,"$4").length;c.html(E(z))}else z(function(q){K=q.replace(Y,"$4").length;c.html(E(q))})}}();l.extend(p,{name:function(c){if(c!==B){M=c;D=new na(c)}else return M},history:function(){return D},set:function(c,q){if(c!==B){a=c;if(!q)o=a.length;x();if(typeof d.onCommandChange=="function")d.onCommandChange(a)}},insert:function(c,q){if(o==a.length)a+=c;else a=o===0?c+a:a.slice(0,o)+c+a.slice(o);q||(o+=c.length);x();if(typeof d.onCommandChange=="function")d.onCommandChange(a)},
...
c;else{c=p.width();var q=k.innerWidth();A=Math.floor(c/q)}x()},enable:function(){if(!L){k.addClass("inverted");p.everyTime(500,"blink",e);L=true}},isenabled:function(){return L},disable:function(){if(L){p.stopTime("blink",e);k.removeClass("inverted");L=false}},mask:function(c){if(typeof c=="boolean"){V=c;x()}else return V}});p.name(d.name||"");z=d.prompt||"> ";H();if(d.enabled===B||d.enabled===true)p.enable();l(l.browser.msie?document.documentElement:window).keypress(function(c){var q;if(c.ctrlKey&&
c.which==99)return true;if(!G&&d.keypress)q=d.keypress(c);if(q===B||q){if(L)if([38,32,13,0,8].has(c.which)&&c.keyCode!=123&&!(c.which==38&&c.shiftKey))return false;else if(!c.ctrlKey&&!(c.altKey&&c.which==100)){if(G){F+=String.fromCharCode(c.which);f();i()}else p.insert(String.fromCharCode(c.which));return false}else if(c.altKey)if(G){F+=String.fromCharCode(c.which);f();i()}else p.insert(String.fromCharCode(c.which))}else return q}).keydown(C);return p};l.jrpc=function(d,e,f,i,m,y){e=l.json_stringify({jsonrpc:"2.0",
...
d?[d]:[],f=0;l.extend(this,{rotate:function(){if(e.length==1)return e[0];else{if(f==e.length-1)f=0;else++f;return e[f]}},length:function(){return e.length},set:function(i){for(var m=e.length;m--;)if(e[m]===i){f=m;return}this.append(i)},front:function(){return e[f]},append:function(i){e.push(i)}})};l.fn.terminal=function(d,e){function f(){return a.get(0).scrollHeight>a.innerHeight()}function i(){var b=l(this).find(".cursor").width(),h=Math.floor(a.width()/b);if(f()){var g=a.innerWidth()-a.width();
h-=Math.ceil((20-g/2)/(b-1))}return h}function m(b,h){a.error("&#91;"+h+"&#93;: "+(typeof b=="string"?b:b.fileName+": "+b.message));a.pause();typeof b.fileName=="string"&&l.get(b.fileName,function(g){a.resume();var n=b.lineNumber-1;(g=g.split("\n")[n])&&a.error("&#91;"+b.lineNumber+"&#93;: "+g)})}function y(b,h){try{if(typeof h=="function")h(function(){});else if(typeof h!="string")throw b+" must be string or function";}catch(g){m(g,b.toUpperCase());return false}return true}function C(){var b=a.prop?
a.prop("scrollHeight"):a.attr("scrollHeight");a.scrollTop(b)}function p(b){b=typeof b=="string"?b:String(b);var h,g,n;if(b.length>M){h=M;g=b.split(/\n/g);n=/(\[\[[bius]*;[^;]*;[^\]]*\][^\]\[]*\]?)/g;var j=/(\[\[[bius]*;[^;]*;[^\]]*\])/,t=/\[\[[bius]*;?[^;]*;?[^\]]*\]?$/,s=false,v=false,u="";b=[];for(var I=0,Z=g.length;I<Z;++I){if(u!=="")if(g[I]===""){b.push(u+"]");continue}else{g[I]=u+g[I];u=""}else if(g[I]===""){b.push("");continue}for(var W=g[I],ea=0,aa=0,R=0,fa=W.length;R<fa;++R){if(W[R]=="["&&
W[R+1]=="[")s=true;else if(s&&W[R]=="]")if(v)v=s=false;else v=true;else if(s&&v||!s)++aa;if(aa==h||R==fa-1){var O=W.substring(ea,R+1);if(u){O=u+O;if(O.match("]"))u=""}ea=R+1;aa=0;var S=O.match(n);if(S){S=S[S.length-1];if(S[S.length-1]!="]"){u=S.match(j)[1];O+="]"}else if(O.match(t)){O=O.replace(t,"");u=S.match(j)[1]}}b.push(O)}}}h=l("<div></div>");g=0;for(n=b.length;g<n;++g)b[g]===""||b[g]=="\r"?h.append("<div>&nbsp;</div>"):l("<div/>").html(E(b[g])).appendTo(h)}else h=l("<div/>").html(E(b));z.append(h);
h.width("100%");C();return h}function J(b,h){var g=1,n=function(j,t){h.pause();l.jrpc(b,g++,j,t,function(s){if(s.error)h.error("&#91;RPC&#93; "+s.error.message);else if(typeof s.result=="string")h.echo(s.result);else if(s.result instanceof Array)h.echo(s.result.join(" "));else if(typeof s.result=="object"){var v="",u;for(u in s.result)if(s.result.hasOwnProperty(u))v+=u+": "+s.result[u]+"\n";h.echo(v)}h.resume()},function(s,v){h.error("&#91;AJAX&#93; "+v+" - Server reponse is: \n"+s.responseText);
h.resume()})};return function(j,t){if(j!==""){var s,v;if(j.match(/[^ ]* /)){j=j.split(/ +/);s=j[0];v=j.slice(1)}else{s=j;v=[]}if(!k.login||s=="help")n(s,v);else{var u=t.token();u?n(s,[u].concat(v)):t.error("&#91;AUTH&#93; Access denied (no token)")}}}}function A(b){var h=r.prompt();if(r.mask())b=b.replace(/./g,"*");typeof h=="function"?h(function(g){a.echo(g+b)}):a.echo(h+b)}function K(b){try{var h=w.top();if(b=="exit"&&k.exit)if(w.size()==1)if(k.login)F();else{A(b);a.echo("You can exit from main interpeter")}else a.pop("exit");
else{A(b);b=="clear"&&k.clear?a.clear():h.eval(b,a)}}catch(g){m(g,"USER");a.resume();throw g;}}function G(){var b=null;r.prompt("login: ");k.history&&r.history().disable();r.commands(function(h){try{A(h);if(b){r.mask(false);a.pause();if(typeof k.login!="function")throw"Value of login property must be a function";k.login(b,h,function(n){if(n){var j=k.name;j=j?"_"+j:"";l.Storage.set("token"+j,n);l.Storage.set("login"+j,b);r.commands(K);U()}else{a.error("Wrong password try again");r.prompt("login: ");
b=null}a.resume();k.history&&r.history().enable()})}else{b=h;r.prompt("password: ");r.mask(true)}}catch(g){m(g,"LOGIN",a);throw g;}})}function F(){var b=k.name;b=b?"_"+b:"";l.Storage.remove("token"+b,null);l.Storage.remove("login"+b,null);k.history&&r.history().disable();G()}function Q(){var b=w.top(),h="";if(b.name!==B&&b.name!=="")h+=b.name+"_";h+=L;r.name(h);b.prompt.constructor==Function?r.prompt(function(g){b.prompt(g,a)}):r.prompt(b.prompt);k.history&&r.history().enable();r.set("");if(typeof b.onStart==
...
1&&a.focus(false);return false}else if(b.which==34)a.scroll(a.height());else b.which==33?a.scroll(-a.height()):a.attr({scrollTop:a.attr("scrollHeight")})}}var a=this,o=[],z,L=N.length(),M,D=[],k={name:"",prompt:"> ",history:true,exit:true,clear:true,enabled:true,cancelableAjax:true,login:null,tabcompletion:null,historyFilter:null,onInit:null,onExit:null,keypress:null,keydown:null};if(e){e.width&&a.width(e.width);e.height&&a.height(e.height);l.extend(k,e)}var x=!k.enabled;if(a.length===0)throw'Sorry, but terminal said that "'+
...
a.enable();else if(b===false)a.next();else{N.front().disable();N.set(a);a.enable()}});return a},enable:function(){M===B&&a.resize();if(x)if(r){r.enable();x=false}return a},disable:function(){if(r){x=true;r.disable()}return a},enabled:function(){return x},signature:function(){var b=a.cols();b=b<15?null:b<35?0:b<55?1:b<64?2:b<75?3:4;return b!==null?oa[b].join("\n")+"\n":""},version:function(){return"0.4.16"},get_command:function(){return r.get()},insert:function(b){if(typeof b=="string"){r.insert(b);
return a}else throw"insert function argument is not a string";},set_prompt:function(b){if(y("prompt",b)){b.constructor==Function?r.prompt(function(h){b(h,a)}):r.prompt(b);w.top().prompt=b}return a},get_prompt:function(){return w.top().prompt},set_command:function(b){r.set(b);return a},set_mask:function(b){r.mask(b);return a},get_output:function(b){return b?o:l.map(o,function(h,g){return typeof g=="function"?g():g}).get().join("\n")},resize:function(b,h){b&&a.width(b);h&&a.height(h);M=i();r.resize(M);
var g=z.detach();z.html("");l.each(o,function(n,j){p(j.constructor==Function?j():j)});a.prepend(g);C();return a},echo:function(b){o.push(b);return p(typeof b=="function"?b():b)},error:function(b){a.echo("[[;#f00;]"+b.replace(/\[/g,"&#91;").replace(/\]/g,"&#93;")+"]")},scroll:function(b){var h;if(a.prop){b>a.prop("scrollTop")&&b>0&&a.prop("scrollTop",0);h=a.prop("scrollTop");a.prop("scrollTop",h+b)}else{b>a.attr("scrollTop")&&b>0&&a.attr("scrollTop",0);h=a.attr("scrollTop");a.attr("scrollTop",h+b)}return a},
logout:k.login?function(){for(;w.size()>1;)w.pop();F();return a}:function(){throw"You don't have login function";},token:k.login?function(){var b=k.name;return l.Storage.get("token"+(b?"_"+b:""))}:null,login_name:k.login?function(){var b=k.name;return l.Storage.get("login"+(b?"_"+b:""))}:null,name:function(){return k.name},push:function(b,h){if(!h.prompt||y("prompt",h.prompt)){if(typeof b=="string")b=J(h.eval,a);w.push(l.extend({eval:b},h));Q()}return a},pop:function(b){b!==B&&A(b);if(w.top().name===
k.name){if(k.login){F();if(typeof k.onExit=="function")k.onExit(a)}}else{b=w.pop();Q();if(typeof b.onExit=="function")b.onExit(a)}return a}});var H=0,c=f(),q;if(k.login&&typeof k.onBeforeLogin=="function")k.onBeforeLogin(a);if(d.constructor==String){q=d;d=J(d,a)}else if(d.constructor==Array)throw"You can't use array as eval";else if(typeof d=="object"){for(var P in d)D.push(P);d=function b(h){return function(g){if(g!==""){g=g.split(/ +/);var n=g[0],j=g.slice(1);g=h[n];var t=typeof g;if(t=="function")g.apply(a,
j);else if(t=="object"||t=="string"){j=[];if(t=="object"){for(var s in g)j.push(s);g=b(g)}a.push(g,{prompt:n+"> ",name:n,command_list:j})}else a.error("Command '"+n+"' Not Found")}}}(d)}else if(typeof d!="function")throw'Unknow object "'+String(d)+'" passed as eval';if(q&&(typeof k.login=="string"||k.login))k.login=function(b){var h=1;return function(g,n,j){a.pause();l.jrpc(q,h++,b,[g,n],function(t){a.resume();!t.error&&t.result?j(t.result):j(null)},function(t,s){a.resume();a.error("&#91;AJAX&#92; Response: "+
s+"\n"+t.responseText)})}}(typeof k.login=="boolean"?"login":k.login);if(y("prompt",k.prompt)){var w=new ma({name:k.name,eval:d,prompt:k.prompt,command_list:D,greetings:k.greetings}),r=a.find(".terminal-output").next().cmd({prompt:k.prompt,history:k.history,historyFilter:k.historyFilter,width:"100%",keydown:V,keypress:k.keypress?function(b){return k.keypress(b,a)}:null,onCommandChange:function(b){if(typeof k.onCommandChange=="function")k.onCommandChange(b,a);C()},commands:K});N.append(a);k.enabled===
true?a.focus():a.disable();l(window).resize(a.resize);a.click(function(){a.focus()});a.token&&!a.token()&&a.login_name&&!a.login_name()?G():U();typeof l.fn.init.prototype.mousewheel==="function"&&a.mousewheel(function(b,h){h>0?a.scroll(-40):a.scroll(40);return false},true)}a.data("terminal",a);return a}})(jQuery);
