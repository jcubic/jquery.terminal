...
"(reverse-i-search)`"+F+"': ";H()}function i(c){var q=D.data(),P=RegExp("^"+F),w=q.length;if(c&&Q>0)w-=Q;for(c=w;c--;)if(P.test(q[c])){Q=q.length-c;o=0;p.set(q[c],true);x();break}}function m(c){var q=c.substring(0,A-K);c=c.substring(A-K);return[q].concat(X(c,A))}function y(){J.focus();p.oneTime(1,function(){p.insert(J.val());J.blur().val("")})}function C(c){if(d.keydown&&d.keydown(c)===false)return false;if(L){var q;if(G&&(c.which==35||c.which==36||c.which==37||c.which==38||c.which==39||c.which==
...
a[q+1]!=" "){c=q+1;break}else if(a[q]=="\n"&&a[q+1]!="\n"){c=q;break}p.position(c)}else{if(o>0){--o;x()}}else if(c.which==82&&c.ctrlKey)if(G)i(true);else{U=z;f();a="";x();G=true}else if(c.which==39||c.which==70&&c.ctrlKey)if(c.ctrlKey&&c.which!=70){a[o]==" "&&++o;c=a.slice(o).match(/\S[\n\s]{2,}|[\n\s]+\S?/);if(!c||c[0].match(/^\s+$/))o=a.length;else if(c[0][0]!=" ")o+=c.index+1;else{o+=c.index+c[0].length-1;c[0][c[0].length-1]!=" "&&--o}x()}else{if(o<a.length){++o;x()}}else if(c.which==123)return true;
else if(c.which==36)p.position(0);else if(c.which==35)p.position(a.length);else if(c.ctrlKey||c.metaKey)if(c.shiftKey){if(c.which==84)return true}else if(c.which==65)p.position(0);else if(c.which==69)p.position(a.length);else if(c.which==88||c.which==67||c.which==87||c.which==84)return true;else if(c.which==86){y();return true}else if(c.which==75)if(o===0)p.set("");else o!=a.length&&p.set(a.slice(0,o));else if(c.which==85){p.set(a.slice(o,a.length));p.position(0)}else{if(c.which==17)return true}else return true;
return false}}var p=this;p.addClass("cmd");p.append('<span class="prompt"></span><span></span><span class="cursor">&nbsp;</span><span></span>');var J=l("<textarea/>").addClass("clipboard").appendTo(p);d.width&&p.width(d.width);var A,K,G=false,F="",Q=null,U,V=d.mask||false,a="",o=0,z,L=d.enabled,M,D,k=p.find(".cursor"),x=function(c){function q(g,n){if(n==g.length){b.html(E(g));k.html("&nbsp;");h.html("")}else if(n===0){b.html("");k.html(E(g.slice(0,1)));h.html(E(g.slice(1)))}else{var j=E(g.slice(0,
n));b.html(j);j=g.slice(n,n+1);k.html(j==" "?"&nbsp;":E(j));n==g.length-1?h.html(""):h.html(E(g.slice(n+1)))}}function P(g){return"<div>"+E(g)+"</div>"}function w(g){var n=h;l.each(g,function(j,t){n=l(P(t)).insertAfter(n).addClass("clear")})}function r(g){l.each(g,function(n,j){b.before(P(j))})}var b=k.prev(),h=k.next();return function(){var g=V?a.replace(/./g,"*"):a,n;c.find("div").remove();b.html("");if(g.length>A-K-1||g.match(/\n/)){var j,t=g.match(/\t/g),s=t?t.length*3:0;if(t)g=g.replace(/\t/g,
