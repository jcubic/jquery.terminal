...
"(reverse-i-search)`"+F+"': ";H()}function i(c){var q=D.data(),P=RegExp("^"+F),w=q.length;if(c&&Q>0)w-=Q;for(c=w;c--;)if(P.test(q[c])){Q=q.length-c;o=0;p.set(q[c],true);x();break}}function m(c){var q=c.substring(0,A-L-1);c=c.substring(A-L-1);return[q].concat(X(c,A))}function y(){J.focus();p.oneTime(1,function(){p.insert(J.val());J.blur().val("")})}function C(c){if(d.keydown&&d.keydown(c)===false)return false;if(M){var q;if(G&&(c.which==35||c.which==36||c.which==37||c.which==38||c.which==39||c.which==
